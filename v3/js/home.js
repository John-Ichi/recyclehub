    ...
    .then(data => {
        ...
        .then(data => {
            ...
            if (banLogs) {
                banLogs.forEach(log => {
                    if (log.unban === "0") {
                        bannedUsers.push(log.userId);
                    }
                });
            }

            const banFilterPosts = posts.filter(post => !bannedUsers.includes(post.userId));
            ...
            filterSelection.forEach(btn => {
                btn.addEventListener("change", () => {
                    ...
                    if (filterArray.length === 0) {
                        renderPosts(banFilterPosts);
                        return;
                    }

                    const filteredPosts = banFilterPosts.filter(post => filterArray.includes(post.category));
